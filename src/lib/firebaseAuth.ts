...
// Provider with full Workspace Gmail Scopes
...
gmailScopes.forEach((scope) => provider.addScope(scope));
  ...
  return onAuthStateChanged(auth, async (user: User | null) => {
    if (user) {
      if (cachedAccessToken) {
        ...
        // User is signed in with Firebase session, but access token needs fresh user popup if expired
